  ...
  <div class="p-2">
    <el-card shadow="hover">
      <el-form :model="query" inline>
        <el-form-item label="渠道名称"><el-input v-model="query.channelName" clearable @keyup.enter="load" /></el-form-item>
      ...
      <div class="toolbar">
        <el-button v-hasPermi="['callcenter:chat-channel:create']" type="primary" plain icon="Plus" @click="openCreate">新增渠道</el-button>
        <span class="hint">渠道键用于网站嵌入，不暴露租户 ID 和后台登录凭证。</span>
      </div>
      <el-table v-loading="loading" :data="rows">
        <el-table-column label="渠道名称" prop="channelName" min-width="170" />
        <el-table-column label="渠道键" min-width="270">
          <template #default="{ row }"><code>{{ row.channelKey }}</code></template>
        </el-table-column>
    ...
    <el-drawer v-model="drawer.visible" :title="drawer.title" size="640px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item label="欢迎语"><el-input v-model="form.welcomeMessage" type="textarea" :rows="3" maxlength="1000" show-word-limit /></el-form-item>
        <el-form-item label="离线提示"><el-input v-model="form.offlineMessage" type="textarea" :rows="3" maxlength="1000" show-word-limit /></el-form-item>
        <el-form-item label="允许嵌入来源">
          <el-input v-model="form.allowedOrigins" type="textarea" :rows="4" placeholder="每行一个完整来源，例如 https://www.example.com；留空表示不限制" />
        </el-form-item>
...
const form = reactive<ChatChannelForm>({ channelName: '', enabled: true });
const embed = reactive({ visible: false, code: '', channelKey: '' });
const rules = { channelName: [{ required: true, message: '请输入渠道名称', trigger: 'blur' }] };
...
onMounted(async () => {
  await Promise.all([loadSkillGroups(), load()]);
});
</script>
