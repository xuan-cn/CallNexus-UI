  ...
  <div class="p-2">
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" :data="queues">
        ...
        <el-table-column label="分配策略" min-width="140"><template #default="{ row }">{{ strategyLabel(row.strategy) }}</template></el-table-column>
        <el-table-column label="最大等待" width="100"><template #default="{ row }">{{ row.maxWaitSeconds }} 秒</template></el-table-column>
        ...
        <el-table-column label="操作" width="200" align="center">
          <template #default="{ row }">
            ...
            <el-button
              v-hasPermi="['callcenter:call-queue:update']"
              link
              type="success"
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="720px" append-to-body>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="队列编码" prop="queueCode"><el-input v-model="form.queueCode" placeholder="例如 SUPPORT_QUEUE" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="队列名称" prop="queueName"><el-input v-model="form.queueName" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="FreeSWITCH节点组" prop="nodeGroupId"><el-select v-model="form.nodeGroupId" style="width: 100%"><el-option v-for="item in nodeGroups" :key="item.id" :label="item.groupName" :value="item.id" /></el-select></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="接听技能组" prop="skillGroupId"><el-select v-model="form.skillGroupId" style="width: 100%"><el-option v-for="item in skillGroups" :key="item.id" :label="item.groupName" :value="item.id" /></el-select></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="分配策略" prop="strategy"><el-select v-model="form.strategy" style="width: 100%"><el-option v-for="item in strategies" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="队列等待音"><el-select v-model="form.waitMediaId" clearable style="width: 100%"><el-option v-for="item in mediaOptions" :key="item.id" :label="item.assetName" :value="item.id" /></el-select></el-form-item></el-col>
        </el-row>
        <el-collapse>
          <el-collapse-item title="高级参数" name="advanced">
            <el-row :gutter="16">
              <el-col :span="12"><el-form-item label="最大等待"><el-input-number v-model="form.maxWaitSeconds" :min="10" :max="86400" /><span class="unit">秒，逐个分配时必须大于振铃超时</span></el-form-item></el-col>
              <el-col :span="12"><el-form-item label="振铃超时"><el-input-number v-model="form.ringTimeoutSeconds" :min="5" :max="300" /><span class="unit">秒，写入 FreeSWITCH leg_timeout</span></el-form-item></el-col>
              <el-col :span="12"><el-form-item label="最大未接次数"><el-input-number v-model="form.maxNoAnswer" :min="0" :max="100" /><span class="unit">由 CallNexus 控制</span></el-form-item></el-col>
              <el-col :span="12"><el-form-item label="话后整理"><el-input-number v-model="form.wrapUpSeconds" :min="0" :max="3600" /><span class="unit">秒，结束后自动示闲；0 表示立即示闲</span></el-form-item></el-col>
            </el-row>
          </el-collapse-item>
        </el-collapse>
...
const syncLabel = (value: CallQueueVO['syncStatus']) => ({ NOT_SYNCED: '未同步', SYNCED: '已同步', PARTIAL: '部分成功', FAILED: '失败' })[value] || value;
const syncTagType = (value: CallQueueVO['syncStatus']) => value === 'SYNCED' ? 'success' : value === 'PARTIAL' ? 'warning' : value === 'FAILED' ? 'danger' : 'info';
