  ...
  <div class="p-2">
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10">
          <el-col :span="1.5"
            ><el-button v-hasPermi="['callcenter:sip-account:create']" type="primary" plain icon="Plus" @click="handleAdd"
              >新增 SIP 账号</el-button
            ></el-col
          >
      ...
      <el-table v-loading="loading" :data="accountList">
        <el-table-column label="分机号" prop="extension" min-width="110" />
        <el-table-column label="显示名称" prop="displayName" min-width="150" />
        <el-table-column label="FreeSWITCH节点" prop="nodeName" min-width="160"
          ><template #default="{ row }">{{ row.nodeName || '未绑定' }}</template></el-table-column
        >
        <el-table-column label="SIP 域" prop="domain" min-width="200" />
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="520px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="95px">
        ...
        <el-form-item label="分机号" prop="extension"><el-input v-model="form.extension" placeholder="例如 1003" /></el-form-item>
        <el-form-item label="显示名称" prop="displayName"><el-input v-model="form.displayName" placeholder="例如 客服分机1003" /></el-form-item>
        <el-form-item label="SIP 域" prop="domain"><el-input v-model="form.domain" placeholder="FreeSWITCH 域名或 IP" /></el-form-item>
...
const initialForm: SipAccountForm = { nodeId: undefined, extension: '', displayName: '', domain: '', password: '', enabled: true };
const data = reactive<PageData<SipAccountForm, SipAccountQuery>>({
  form: { ...initialForm },
  queryParams: { pageNum: 1, pageSize: 10, extension: '', displayName: '', enabled: undefined },
  rules: {
    ...
    domain: [{ required: true, message: 'SIP域不能为空', trigger: 'blur' }],
    password: [
      {
        validator: (_rule: unknown, value: string, callback: (error?: Error) => void) =>
          !form.value.id && (!value || value.length < 12) ? callback(new Error('密码至少12位')) : callback(),
        trigger: 'blur'
      }
    ]
  }
